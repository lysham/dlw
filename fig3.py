...
if __name__ == "__main__":
    ...
    pw = np.linspace(0.001, 2.5/100, 20)
    b2_h2o = 0.1083 + 0.0748 * np.tanh(270.8944 * pw)
    b3_h2o = -0.2308 + 0.6484 * np.power(pw, 0.1280)
    b4_h2o = 0.0289 + 6.2436 * np.power(pw, 0.9010)
    de_b2_h2o = 20.2629 * np.power(np.cosh(270.8944 * pw), -2)
    de_b3_h2o = (0.6484 * 0.1280) * np.power(pw, 0.1280 - 1)
    de_b4_h2o = (6.2436 * 0.9010) * np.power(pw, 0.9010 - 1)

    b2_co2 = 0.0002
    b3_co2 = 0.3038 - 0.5262 * np.power(pw, 0.1497)
    b4_co2 = 0.0144 - 0.1740 * np.power(pw, 0.7268)
    de_b2_co2 = np.zeros(len(pw))
    de_b3_co2 = (-0.5262 * 0.1497) * np.power(pw, 0.1497 - 1)
    de_b4_co2 = (-0.1740 * 0.7268) * np.power(pw, 0.7268 - 1)
