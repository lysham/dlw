...
def getNodeProperty(NodeID, PropName, NodeList=GLOBAL_NODELIST):
    ...
    if node:
        ...
        if prop:
            ...
            raise Exception(
                "node %s doesn" "t have property %s" % (ModeName, Propname))
    else:
...
GLOBAL_PRIVILEGES = {
    "admin": {"ACCESS_KEY": "6cfd7040-24a6-4197-81f9-6e25e50005b2",
              "LEVEL": "ADMIN"},
    "guest": {"ACCESS_KEY": "9b6a7975-2a84-43d8-920e-f4dea9db6805",
              "LEVEL": "USER"},
}
...
def checkPrivileges(
    ...
    if not checkAccess(
            DBName, TableName, NodeName, UserName, Requisites, NodeList,
            NodeNames
    ):
        return False
    return True
...
def getFullTableAndHeaderName(TableName):
    ...
    fullpath_data = VARIABLES[
                        "BACKEND_DATABASE_NAME"] + "/" + TableName + ".data"
    if not os.path.isfile(fullpath_data):
        fullpath_data = VARIABLES[
                            "BACKEND_DATABASE_NAME"] + "/" + TableName + ".par"
        if not os.path.isfile(fullpath_data) and TableName != "sampletab":
            raise Exception('Lonely header "%s"' % fullpath_data)
    fullpath_header = VARIABLES[
                          "BACKEND_DATABASE_NAME"] + "/" + TableName + ".header"
...
def addRowObject(RowObject, TableName):
    ...
    if [p[0] for p in RowObject] != LOCAL_TABLE_CACHE[TableName]["header"][
        "order"]:
...
def cache2storage(TableName):
    ...
    for RowID in range(0, LOCAL_TABLE_CACHE[TableName]["header"][
        "number_of_rows"]):
        line_count += 1
...
def evaluateExpression(root, VarDictionary, GroupIndexKey=None):
    ...
    if type(root) in set([list, tuple]):
        ...
        for element in tail:  # resolve tree by recursion
            args.append(
                evaluateExpression(element, VarDictionary, GroupIndexKey))
        # call functions with evaluated arguments
        try:
...
def describeTable(TableName):
    ...
    print("Table type: " + str(
        LOCAL_TABLE_CACHE[TableName]["header"]["table_type"]))
    print("-----------------------------------------")
...
def selectInto(DestinationTableName, TableName, ParameterNames, Conditions):
    ...
    for RowID in range(0, table_length):
        ...
        if checkRowObject(RowObject, Conditions, VarDictionary):
            ...
            row_count += 1
    LOCAL_TABLE_CACHE[DestinationTableName]["header"][
...
def sort(
    ...
    if not ParameterNames:
        ...
        ParameterNames = [
            ParameterNames]  # fix of stupid bug where ('p1',) != ('p1')
...
def group(
    ...
    for RowID in range(0, number_of_rows):
        ...
        RowObject = getRowObject(RowID,
                                 TableName)  # RowObject from source table
        VarDictionary = getVarDictionary(RowObject)
...
def extractColumns(
        TableName, SourceParameterName, ParameterFormats, ParameterNames=None,
        FixCol=False
):
    """
    INPUT PARAMETERS:
    ...
    if type(
            LOCAL_TABLE_CACHE[TableName]["header"]["default"][
                SourceParameterName]
...
PARLIST_SDVOIGT_AIR = ["gamma_air", "delta_air", "deltap_air", "n_air",
                       "SD_air"]
PARLIST_SDVOIGT_SELF = ["gamma_self", "delta_self", "deltap_self", "n_self",
                        "SD_self"]
PARLIST_SDVOIGT_H2 = []
PARLIST_SDVOIGT_CO2 = []
PARLIST_SDVOIGT_HE = []
...
PARLIST_GALATRY_AIR = ["gamma_air", "delta_air", "deltap_air", "n_air",
                       "beta_g_air"]
...
def queryHITRAN(
    ...
    ParameterList = prepareParlist(pargroups=pargroups, params=params,
                                   dotpar=dotpar)
    TableHeader = prepareHeader(ParameterList)
    TableHeader["table_name"] = TableName
    DataFileName = VARIABLES[
                       "BACKEND_DATABASE_NAME"] + "/" + TableName + ".data"
    HeaderFileName = VARIABLES[
                         "BACKEND_DATABASE_NAME"] + "/" + TableName + ".header"
    # create URL
...
ISO_ID_INDEX = {"M": 0, "I": 1, "iso_name": 2, "abundance": 3, "mass": 4,
                "mol_name": 5}
    ...
    if abs(c2t) == 0.0e0:
        ...
        if any(index_Z1):
            Bterm_GLOBAL = (
                    rpi
                    * cte
                    * ((1.0e0 - Z1 ** 2) * __ComplexType__(
                WR1 + 1.0e0j * WI1) + Z1 / rpi)
            )
        if any(index_NOT_Z1):
            Bterm_GLOBAL = cte * (
        ...
        if any(index_PART3):
            ...
            if any(index_ABS):
                ...
                Aterm[index_ABS] = (2.0e0 * rpi / c2t) * (
                        1.0e0 / rpi
                        - sqrt(X_TMP[index_ABS]) * __ComplexType__(
                    WRb + 1.0e0j * WIb)
                )
...
def EnvironmentDependency_Intensity(
        LineIntensityRef, T, Tref, SigmaT, SigmaTref, LowerStateEnergy,
        LineCenter
):
    const = __FloatType__(1.4388028496642257)
    ch = exp(-const * LowerStateEnergy / T) * (1 - exp(-const * LineCenter / T))
    zn = exp(-const * LowerStateEnergy / Tref) * (
                1 - exp(-const * LineCenter / Tref))
    ...
    for TableName in SourceTables:
        ...
        for RowID in range(nline):
            ...
            for parname in parnames:
                Line[parname] = LOCAL_TABLE_CACHE[TableName]["data"][parname][
                    RowID]
            ...
            LowerStateEnergyDB = LOCAL_TABLE_CACHE[TableName]["data"]["elower"][
                RowID]
            MoleculeNumberDB = LOCAL_TABLE_CACHE[TableName]["data"]["molec_id"][
                RowID]
            IsoNumberDB = LOCAL_TABLE_CACHE[TableName]["data"]["local_iso_id"][
                RowID]
            ...
            for species in Diluent:
                ...
                try:
                    ...
                    try:
                        ...
                        if VARIABLES["DEBUG"]:
                            print(
                                "absorptionCoefficient_HT: Gamma2DB = SDDB*Gamma0DB")
                    except KeyError:
                        Gamma2DB = 0.0
                        if VARIABLES["DEBUG"]:
                            print(
    ...
    for TableName in SourceTables:
        ...
        for RowID in range(nline):
            ...
            for species in Diluent:
                ...
                try:
                    Gamma0DB = LOCAL_TABLE_CACHE[TableName]["data"][gamma_name][
                        RowID]
                except:
                    Gamma0DB = 0.0
                ...
                try:
                    TempRatioPowerDB = \
                    LOCAL_TABLE_CACHE[TableName]["data"][n_name][
                        RowID
                    ]
                    if species_lower == "self" and TempRatioPowerDB == 0.:
                        TempRatioPowerDB = LOCAL_TABLE_CACHE[TableName]["data"][
                    ...
                    # TempRatioPowerDB = 0
                    TempRatioPowerDB = \
                    LOCAL_TABLE_CACHE[TableName]["data"]["n_air"][
                        RowID
                    ]
                ...
                try:
                    Shift0DB = LOCAL_TABLE_CACHE[TableName]["data"][delta_name][
                        RowID]
                except:
                    Shift0DB = 0.0
                ...
                try:
                    deltap = LOCAL_TABLE_CACHE[TableName]["data"][deltap_name][
                        RowID]
                except:
                    deltap = 0.0
                ...
                Gamma2 += (
                        abun
                        * CustomEnvDependences.get(SD_name,
                                                   SDDB * p / pref)  # default ->
                        * Gamma0DB
                )
    ...
    for TableName in SourceTables:
        ...
        for RowID in range(nline):
            ...
            IsoNumberDB = LOCAL_TABLE_CACHE[TableName]["data"]["local_iso_id"][
                RowID]
            if LineShift:
                Shift0DB = LOCAL_TABLE_CACHE[TableName]["data"]["delta_air"][
                    RowID]
            else:
                Shift0DB = 0
...
def abscoef_HT(table=None, step=None, grid=None, env={"T": 296., "p": 1.},
               file=None):
    return absorptionCoefficient_HT(
        SourceTables=table, OmegaStep=step, OmegaGrid=grid, Environment=env,
        File=file
    )
...
def convolveSpectrumFull(
        Omega, CrossSection, Resolution=0.1, AF_wing=10.,
        SlitFunction=SLIT_RECTANGULAR
):
    """
    Convolves cross section with a slit function with given parameters.
