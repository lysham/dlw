...
if __name__ == "__main__":
    ...
    site_elev = SURFRAD[site]["alt"] / 1000  # km
    p = np.interp(site_elev, af_sum.alt_km.values, af_sum.pres_mb.values)  # mb
    # tau_shp_bou = evaluate_sr2021(x, site_param=site, p_param=P_ATM)
    ...
    # add data for GWC (circle)
    pdf = data.loc[data.site == "GWC"].sample(100, random_state=14)
    ax.scatter(
        (pdf.pw_hpa * 100) / P_ATM, -1 * np.log(1-pdf.y), marker="o", s=ms,
        alpha=0.3, c="0.3", ec="0.5", lw=0.5, zorder=0
    )

    # add data for TBL (triangle)
    pdf = data.loc[data.site == "BOU"].sample(100, random_state=14)
    ax.scatter(
        (pdf.pw_hpa * 100) / P_ATM, -1 * np.log(1-pdf.y), marker="^", s=ms,
        alpha=0.3, c="0.3", ec="0.5", lw=0.5, zorder=0
    )
    fit_label = f"${C1_CONST:.03f}+{C2_CONST:.03f}$" + "$\sqrt{p_w}$"
    ax.plot(x, -1 * np.log(y_fit), lw=2, ls="-", c="0.0", zorder=0,
    ...
    # tmp = df.total.to_numpy() / df.aerosols.to_numpy()
    # ax.plot(
    #     x, -1 * np.log(tmp), c=COLORS["persianred"],
    #     ls=":", label="LC2019 no aero", zorder=4
    # )
    # tmp = df.H2O.to_numpy() * df.CO2.to_numpy()
    # ax.plot(
    #     x, -1 * np.log(tmp), c=COLORS["persianred"],
    #     ls="-.", label="LC2019 H2O+CO2", zorder=4
    # )
    #
    # eps = ijhmt_to_individual_e()
